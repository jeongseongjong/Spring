...
public class TourAppService {
	...
	// 시,도 리스트를 가져오는 query문자열 + 시군구 리스트를 가져오는 query문자열 추가
	...
	public List<CityVO> getAreaData(String cityCode) throws JsonSyntaxException, IOException {
		...
		// 이 클래스에서 만든 query문자열을 
		// tgService의 getDataString()에 보내서
		// 데이터 조회를 하는 메소드
		String resString = "";
		if(cityCode == null) {
		...
		/*
		 * gson에 있는 class JsonArray 데이터를 List형 데이터로 변환하는 도구
		 * 
		 * 1. JsonArray를 List형으로 변환하기 위한 바구니 생성 2. Gson을 이용하여 List형으로 변환
		 */
		TypeToken<List<CityVO>> cityToken = new TypeToken<List<CityVO>>() {
		};

		// token도구를 사용하여 List<Class> 형으로 변환하는 메소드
		Gson gson = new Gson();
