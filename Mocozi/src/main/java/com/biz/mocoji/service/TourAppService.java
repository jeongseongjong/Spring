...
public class TourAppService {
	...
	public String makeAreaBaseQuery(String areaCode, int numOfRows) throws UnsupportedEncodingException {
		
		String AreaBaseQuery = this.getHeaderQuery("searchFestival");
		
		AreaBaseQuery += "&arrange=R";
		...
		AreaBaseQuery += String.format("&pageNo=%d", 1);		// 페이지 번호
		
		return AreaBaseQuery;
	}
	public List<AreaBaseDTO> getAreaBase(String areaCode, int numOfRows) throws UnsupportedEncodingException, IOException{
		
		log.debug("보내기전 areaCode : " + areaCode);
		log.debug("보내기전 makeAreaBaseQuery : " + this.makeAreaBaseQuery(areaCode, numOfRows));
		
		String jsonQuery = tgService.getQuery(this.makeAreaBaseQuery(areaCode, numOfRows));
	...
	public String makeSearchQuery(String keyword) throws UnsupportedEncodingException {
		
		String searchQuery = this.getHeaderQuery("searchKeyword");
		searchQuery += "&arrange=R";
		searchQuery += "&contentTypeId=15";
		searchQuery += "&keyword=" + URLEncoder.encode(keyword, "UTF-8");
		searchQuery += String.format("&numOfRows=%d", 100); 
		searchQuery += String.format("&pageNo=%d", 1);	
		
		return searchQuery;
	}
	public List<AreaBaseDTO> getSearchList(String keyword) throws UnsupportedEncodingException, IOException{
		
		String jsonQuery = tgService.getQuery(this.makeSearchQuery(keyword));
