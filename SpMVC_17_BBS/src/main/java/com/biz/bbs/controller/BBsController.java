...
public class BBsController {
	 ...
	 * 별도로 저장하지 않아도 답글을 저장했을때 원글의 내용이 
	 * 같이 controller로 전송되도록 하기위한 설정
	 * 
	 * @ModelAttribute로 설정된 bbsVO는
	 * sessionAttributes에 설정이 되어 있기 때문에
	 * 
	 * model.addAttribute로 만드는 순간 서버 session메모리에 데이터가
	 * 통째로 저장되어 있어서 다른 method에서 그 값을 참조할 수 있다.
	 */
	...
	public String comment(CommentVO cmmVO, Model model) {
		...
		log.debug("요기는 comment 창 : " + cmmVO.toString());
