...
public class UserController {
	...
	// UserService uService = new UserServiceImp()와 같다
	@Autowired
	...
	public String join(Model model) {
		...
		/*
		 * insert.jsp에서 spring form tag를 사용하면서 modelAttribute를 설정해 두었는데 
		 * 해당 Attribute를 전달해주지 않으면 form을 열때 오류가 발생을 한다. 
	...
	@RequestMapping(value = "/join", method = RequestMethod.POST)
												// @valid : DTO에서 정보를 받아 bResult에 주입한다.
	 ...
	 * 사용자가 strText에 값을 전달하지 않아도 400오류를 내지말아라
	 * 
	 * ! 절대 DTO, VO에는 설정 금지
	 * required = false, 
	 * 
	 ...
	 * ResolverView에게 보내서 *.jsp 파일과 렌더링 하는일을 하지말아라
	 * 
	 * 	문자열을 있는 그대로 브라우저로 전송
	 * @ResponseBody가 없으면
	 * 	통상 /views/???.jsp 파일을 렌더링하여 
	 * 	브라우저로 전송하라
	 * 
	 */
