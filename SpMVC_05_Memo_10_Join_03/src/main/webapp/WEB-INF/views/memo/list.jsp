...
<%
/*
	css, js 파일을 만들고 jsp, html에서 import를 하여 사용할 경우
	해당 파일을 변경(수정)했을 경우 tomcat이 해당파일이 변경 된것을
	알지 못하는 현상 발생
	
	이럴경우 브라우저의 방문기록을 삭제하면 변경된 내용이 반영된다
	상당히 불편하고 만약 배포된 상태에서 불특정 다수가 웹을 통해
	접속했을 경우 사용자에게 일일이 브라우저 방문기록을 삭제해달라 요청할수 없다
	
	그래서 파일이름을 변경하여 서버를 reloading하는 방법이 있는데
	이 방법도 계속해서 파일이름을 변경해야하는 상황이 좋지 않다.
	...
	?ver-***라는 query를 부착하여
	버전숫자를 변경하고
	tomcat을 reloading해주면 문제가 해결된다
	
	따라서
	css, js파일의 코드를 변경한 후에는
	ver 숫자를 지금과 다른 값으로 변경하고 tomcat을 reloading 하자
*/
%>
	<link rel="stylesheet" type="text/css"
		href="${rootPath}/css/list-table.css?ver=2019-12-03">
	<table>
		<tr>
			<th>SQ</th>
