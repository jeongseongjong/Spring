...
<script>
	$(function() {
		...
		// 버튼 클릭이라는 이벤트가 발생했을 때 아래의 코드가 실행된다
		// 어떤 이벤트가 발생했을 때 실행되는 소스 : call back
		$("#btn-search").click(function() {

			// $("form") = form에 있는 데이터를 가져오겠다.
			// from에 입력된 데이터를 json데이터 타입을 바꾸어서
			// 서버에 전송할 수 있도록 문자열화 하여라
			var queryString = $("form").serialize();

			/*
				Async JavaScript XML
				비동기 방식으로 서버에 request를 보낼 때 사용하는
				JS의 내장 프로토콜
			...
			$.ajax({
				...
				url : "${rootPath}/searchAPI", // 서버로 request할 주소
				data : queryString, // 서버로 보낼 데이터
				type : 'post', // req할 method
				// dataType : 'json', // response로 받을 데이터 타입
				success : function(result) { // 결과가 성공했을 때
					// resFunc(result)
					// $("bok-list")에다가 html결과를 그대로 갖다 붙여라
					$("#bok-list").html(result)
				},
				error : function(xhr, statux, error) { // 서버에서 오류를 보낼때
					alert(error)
				}
			...
			var resFunc = function(result) {
				...
				result.forEach(function(vo) {
					// $("p").text(vo.servDgst)를 temp에 저장해라
					temp.append($("<p/>", {
						text : vo.servDgst,
						dataId : vo.servDgst
					}))
