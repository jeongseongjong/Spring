...
public interface ImageDao {
	...
	@Select("SELECT * FROM tbl_gallery "
			...
			+ "img_file = #{img_file}")
	public ImageVO findBySeqAndFile(
			@Param("img_seq")long img_seq, 
			@Param("img_file")String img_file);
	
	
	
	
//	@SelectKey(keyProperty  = "img_seq", 
//			keyColumn = "img_seq", resultType = Long.class, 
//			statement = "SELECT * FROM tbl_gallery WHERE img_seq = #{img_seq}", before=false)
	...
	@Select("SELECT * FROM tbl_images "
			+ "WHERE img_file_p_code = #{img_file_p_code}")
