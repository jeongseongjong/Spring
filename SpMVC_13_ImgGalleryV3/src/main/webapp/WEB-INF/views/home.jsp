...
<script src="${rootPath}/javascript/summernote-ko-KR.js"
...
.img_card {
	...
	height: 200px;
	margin: 10px;
	/* 이미지가 카드박스보다 클때 숨기는 기능 */
	overflow: hidden;
	display: flex;
	justify-content: center;
	flex-flow: column;
	box-shadow: 0 2px 5px 0 rgba(0, 0, 0, 0.16), 0 4px 20px 0
		rgba(0, 0, 0, 0.19);
}
...
*/			
		...
		/*
			jquery 응용
			마우스를 제어해서 context menu(오른쪽 클릭 메뉴)
			만들어주는 tool
		 */
		...
		var contextCallBack = function(key, options) {
			...
			if (key == 'delete') {
				...
				if (confirm("이미지를 삭제할까유")) {
					let img_seq = $(this).attr("data-seq");

					$.ajax({
						url : "${rootPath}/image/delete",
						// data : img_seq라고 하나만 해도 되지만 가끔 오류가 생긴다
						// data : img_seq = 3 
						data : {
							img_seq : img_seq
						},
		...
		var img_context = function(key, option){
			...
			if (key == 'delete') {
				...
				// 슬래시(/) 문자리를 기준으로 분해gotj
				// 분해된 배열의 제일 마지막 값을 가져오면 = 파일이름만
