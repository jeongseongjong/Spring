...
public class StudentController {
	...
	/*
	 * input form에서 데이터를 입력한 후 전송을 하면
	 * 데이터를 수신할 method
	 * 
	 * 매개변수로 설정된 StudentDTO dto는 
	 * form의 input box에 설정된 
	 * name과 같은 변수명을 필드로 갖는 DTO
	 * 
	 * 별다를 setting조치 업이 Spring 은 자동으로
	 ...
	 * 문자열로 전달받은 데이터를 int형으로 Integer.valueOf() method를 이용해서
	 * 숫자형으로 변환을 시도할 것이다.(Spring framwork에서)
	 * 
	 * 만약 st_grade input box에 값을 입력하지 않고 전송을 누르면
	 * 해당 변수에 담긴 값은 ""이거나 null 일 것이다.
	 * Integer.valueOf("") 또는 Integer.valuesOf(null)를 실행할텐데
	 * NumberformatException이 발생한다.
	 * 
	 * 그럼 톰캣은 web browser에 400 오류를 보내서
	 * 가밧이 잘못 전달되었다는것을 알려준다.
	 */
