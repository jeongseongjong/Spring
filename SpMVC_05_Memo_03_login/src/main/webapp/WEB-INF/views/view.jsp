...
$(function(){
	$("#btn-update").on("click",function(){
		...
		/*
			새 페이지를 만들어 지금 페이지 위에 보여라
			뒤로가기 시 이전 페이지로 돌아간다
		*/
		document.location.href 
			= "${rootPath}/memo/update?id=${memoDTO.m_seq}"  
	...
	$("#btn-delete").click(function(){
		...
		if(confirm("메모를 삭제합니다")){
			/*
			현재 페이지를 지우고 새로운 페이지로 다시 보여라
			뒤로가기시 이상한 페이지(history로 저장된)로 돌아가게 된다.
		*/
...
<body>
	...
	<table>
		...
		<tr>
			<th>제목</th>
			<td colspan="3">${memoDTO.m_subject}</td>
		</tr>

		<tr>
			<th>세부내용</th>
			<td colspan="3">${memoDTO.m_text}</td>
		</tr>
	</table>
	<br />
	<br />
