...
//							.build();
	...
	public String view(@RequestParam("id") String str_seq, @ModelAttribute MemoDTO memoDTO, Model model) {
	...
	/*
	 * 브라우저에서 delete를 호출할때
	 * m_seq변수에 값을 포함하지 않고 req보내면
	 * 서버에서는 400오류가 발생한다
	 * 그 이유는 Spring은 web에서 넘겨져온 데이터를
	 * long형으로 형변환을 수행하여 값을 받으려고 시도한다
	 * 그 과정에서 값이 없으면 null을 long형으로 변환하는 코드가 실행되어
	 * 서버 내부에서 exception이 발생을 하고
	 * 400오류를 web에게 알려주게 된다.
	 * 
	 * 이를 방지하기 위해 String으로 일단 받고
	 * 별도로 Long.valueOf를 실행하는 코드가 안전하다
	 * 
	 * 하지만 delete를 호출할 때 
	 * 절대 m_seq값이 없이는 호출되지 않는다는 가정하에
